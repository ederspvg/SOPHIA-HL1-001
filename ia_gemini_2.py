...
genai_.configure(api_key=os.getenv('GEMINI_API_KEY'))
...
#
# global_ultima_especie_consultada    = '' # Armazena o resultado da última consulta feita à IA
# global_ultima_reino                 = '' # 
# global_ultima_classe                = '' #
# global_utlima_ordem                 = '' #
# global_ultima_familia               = '' #
# global_ultima_filo                  = '' #
# global_ultima_parente               = '' #
# global_ultima_genero                = '' #
...
def analisar_imagem_com_gemini(_image_path, _pdf_path, _contexto, _instrucao, _tentativas_restantes):
    ...
    logic_usa_Chat              = False
    
    # Checa se é para listar os modelos
    #
    if logic_lista_modelos:
        # listar modelos
        for m in genai_.list_models():
            if 'generateContent' in m.supported_generation_methods:
                print(m.name)
        #
    #
    
    # Definie o objeto para pesquisa no Google
    ...
    model = genai_.GenerativeModel('gemini-2.0-flash-exp', system_instruction=_instrucao) # o gemini-1.5-pro é gratuito, mas tem um limite de uso diario. O flash não possui o limite diario: https://developers.googleblog.com/pt-br/gemini-15-pro-and-15-flash-now-available/
    #
    #genai_.GenerativeModel("gemini-2.0-flash-exp",system_instruction=_instrucao,tools=google_search_tool)
    ...
    if _pdf_path == '':
        ...
        print("Há pdf")
        #arquivo_pdf = genai_.upload_file(_pdf_path, display_name="")
        arquivo_pdf = extrair_texto_de_pdf(_pdf_path)
         ...
         try:
             ...
             if logic_usa_Chat:
                ...
                resposta = client.models.generate_content(
                    ...
                    contents=[_prompt, img, arquivo_pdf],
...
if teste:
    ...
    interacao_usuario   = input("O que significa esse print de tela?")
    imagem_             = input("Informe o caminho da imagem: ")
    ...
    instrucao_IA        = 'Você é um assistente de pesquisa na internet. Seja sempre franco e verdadeiro nas suas respostas, mesmo que isso signifique não conseguir ajudar quando não há dados na internet ou quando o usuário solicita algo que você não é capaz de fazer. Mas, caso você seja capaz e caso hajam informações na internet, responda de forma detalhada e mencione as suas fontes de pesquisa.'
    
    #arquivo_pdf         = 'arquivo_temporario.pdf' #'' #'arquivo_temporario.pdf'
    arquivo_pdf         = input("Informe o caminho do PDF: ")
    
    resposta_ChatBox    = analisar_imagem_com_gemini(imagem_, arquivo_pdf, interacao_usuario, instrucao_IA, 10)
